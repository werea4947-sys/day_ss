...
class TranslationDataset(Dataset):
    def __init__(self, en_path, zh_path, en_tokenizer, zh_tokenizer):
    ...
    def __getitem__(self, idx):
        ...
        return {
            'input_ids': en_enc['input_ids'].squeeze(),
            'attention_mask': en_enc['attention_mask'].squeeze(),
            'labels': zh_enc['input_ids'].squeeze()
        }
...
#数据加载
dataset = TranslationDataset('/kaggle/input/zh-and-en/english.txt', '/kaggle/input/zh-and-en/chinese.txt', tokenizer, tokenizer)
...
def evaluate(model, loader, epoch):
    ...
    #计算BLEU-4
    bleu = sacrebleu.corpus_bleu(all_preds, all_labels, tokenize='zh')
